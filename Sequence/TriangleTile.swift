...
class TriangleTile: SKSpriteNode, GameTile {
    var number : Int32
    ...
    let highlight = SKSpriteNode(imageNamed: "TriangleHighlight.png")
    let white = SKSpriteNode(imageNamed: "TriangleWhite.png")
    var numConflicts = 0
    var prepped = false
    var prepping = false
    ...
    required init(gameScene: GameScene, number: Int32, angle: CGFloat) {
        ...
        edgeDist = self.size.width*0.385*3/self.xScale
        bottomDist = self.size.width*0.225*3/self.xScale
    ...
    func rotate() {
        self.runAction(SKAction.rotateByAngle(CGFloat(-M_PI/3), duration: 0.3))
        if (bottomDist == self.size.width*0.44*3/self.xScale){
            bottomDist = self.size.width*0.225*3/self.xScale
        }
        else {
            bottomDist = self.size.width*0.44*3/self.xScale
        }
    }
    ...
    func turnRed() {
        self.texture = SKTexture(imageNamed: "TriangleRed.png")
    }
    
    func turnBlack() {
        self.texture = SKTexture(imageNamed: "Triangle.png")
    }

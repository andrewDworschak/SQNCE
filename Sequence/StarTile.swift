...
class StarTile: SKSpriteNode, GameTile {
    var number : Int32
    ...
    let highlight = SKSpriteNode(imageNamed: "StarHighlight.png")
    let white = SKSpriteNode(imageNamed: "StarWhite.png")
    var numConflicts = 0
    var prepped = false
    var prepping = false
    ...
    required init(gameScene: GameScene, number: Int32, angle: CGFloat) {
        ...
        let texture = SKTexture(imageNamed: "Star.png")
        
        super.init(texture: texture, color: UIColor.whiteColor(), size: CGSize(width: gameScene.frame.height*0.06, height: gameScene.frame.height*0.06))
        edgeDist = self.size.width*0.42*3/self.xScale
        bottomDist = self.size.width*0.37*3/self.xScale
    ...
    func rotate() {
        self.runAction(SKAction.rotateByAngle(CGFloat(-M_PI/5), duration: 0.3))
        if (bottomDist == self.size.width*0.47*3/self.xScale){
            bottomDist = self.size.width*0.37*3/self.xScale
        }
        else {
            bottomDist = self.size.width*0.47*3/self.xScale
        }
    }
    ...
    func turnRed() {
        self.texture = SKTexture(imageNamed: "StarRed.png")
    }
    
    func turnBlack() {
        self.texture = SKTexture(imageNamed: "Star.png")
    }

...
class HexagonTile: SKSpriteNode, GameTile {
    var number : Int32
    ...
    let highlight = SKSpriteNode(imageNamed: "HexagonHighlight.png")
    let white = SKSpriteNode(imageNamed: "HexagonWhite.png")
    var numConflicts = 0
    var prepped = false
    var prepping = false
    ...
    required init(gameScene: GameScene, number: Int32, angle: CGFloat) {
        ...
        let texture = SKTexture(imageNamed: "Hexagon.png")
        ...
        edgeDist = self.size.width*0.47*2.7/self.xScale
        bottomDist = self.size.width*0.41*2.7/self.xScale
        ...
        highlight.size = CGSize(width: self.size.width*2.7, height: self.size.height*2.7)
        highlight.alpha = 0
        white.alpha = 0
        white.size = CGSize(width: self.size.width*2.7, height: self.size.height*2.7)
    ...
    func prepare(x: CGFloat){
        ...
        self.runAction(SKAction.scaleTo(2.7, duration: 0.03))
    ...
    func rotate() {
        self.runAction(SKAction.rotateByAngle(CGFloat(-M_PI/6), duration: 0.3))
        if (edgeDist == self.size.width*0.41*2.7/self.xScale){
            edgeDist = self.size.width*0.47*2.7/self.xScale
            bottomDist = self.size.width*0.41*2.7/self.xScale
        }
        else {
            edgeDist = self.size.width*0.41*2.7/self.xScale
            bottomDist = self.size.width*0.47*2.7/self.xScale
        }
    }
    ...
    func turnRed() {
        self.texture = SKTexture(imageNamed: "HexagonRed.png")
    }
    
    func turnBlack() {
        self.texture = SKTexture(imageNamed: "Hexagon.png")
    }
